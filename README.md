...
| Header | Define before include | Public function | What it does | Notes / requirements |
| --- | --- | --- | --- | --- |
| `pcv_get_filters.h` | `PCV_GET_FILTERS` | `int pcv_get_filters(int n, float *output, char *filter)` | Generates an `n x n` filter kernel into `output`. Supported filter names in the current code are `BOX`, `SOBEL_X`, `SOBEL_Y`, and `GAUSSIAN_BLUR`. | `output` must point to space for `n * n` floats. |
...
| `pcv_normalize.h` | `PCV_NORMALIZE` | No public `pcv_` API yet | Header exists, but there is not currently a complete exposed normalization function in the `extern "C"` section. | Treat this header as incomplete for now. |
